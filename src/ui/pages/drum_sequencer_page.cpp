...
class DrumSequencerMainPage : public Container {
 ...
 void transposePattern(int instrumentDelta); // +1 = up, -1 = down (voice cycling)
...
// === New helpers ===
void DrumSequencerMainPage::rotatePattern(int delta) {
 if (delta == 0) return;
 bool current_hits[NUM_DRUM_VOICES][SEQ_STEPS];
 bool current_accents_v[NUM_DRUM_VOICES][SEQ_STEPS];
 bool current_accent_global[SEQ_STEPS];
 const bool* hits_src[NUM_DRUM_VOICES] = {
 ...
 const bool* acc_src[NUM_DRUM_VOICES] = {
 ...
 const bool* accent_global = mini_acid_.patternDrumAccentSteps();
 for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
  for (int i = 0; i < SEQ_STEPS; ++i) {
    current_hits[v][i] = hits_src[v][i];
    current_accents_v[v][i] = acc_src[v][i];
  }
 }
 for (int i = 0; i < SEQ_STEPS; ++i) current_accent_global[i] = accent_global[i];
 bool target_hits[NUM_DRUM_VOICES][SEQ_STEPS];
 bool target_accents_v[NUM_DRUM_VOICES][SEQ_STEPS];
 bool target_accent_global[SEQ_STEPS];
 const int shift = (delta > 0) ? 1 : -1;
 for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
  for (int i = 0; i < SEQ_STEPS; ++i) {
    int src = (i - shift + SEQ_STEPS) % SEQ_STEPS;
    target_hits[v][i] = current_hits[v][src];
    target_accents_v[v][i] = current_accents_v[v][src];
  }
 }
 for (int i = 0; i < SEQ_STEPS; ++i) {
  int src = (i - shift + SEQ_STEPS) % SEQ_STEPS;
  target_accent_global[i] = current_accent_global[src];
 }
 withAudioGuard([&]() {
  for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
    for (int i = 0; i < SEQ_STEPS; ++i) {
      if (current_hits[v][i] != target_hits[v][i]) {
        mini_acid_.toggleDrumStep(v, i);
      }
      bool desiredAccentV = target_accents_v[v][i] && target_hits[v][i];
      if (current_accents_v[v][i] != desiredAccentV) {
        mini_acid_.setDrumAccentStep(v, i, desiredAccentV);
      }
    }
  }
  for (int i = 0; i < SEQ_STEPS; ++i) {
    if (current_accent_global[i] != target_accent_global[i]) {
      mini_acid_.toggleDrumAccentStep(i);
    }
  }
 });
}

void DrumSequencerMainPage::transposePattern(int instrumentDelta) {
 if (instrumentDelta == 0) return;
 bool current_hits[NUM_DRUM_VOICES][SEQ_STEPS];
 bool current_accents_v[NUM_DRUM_VOICES][SEQ_STEPS];
 const bool* hits_src[NUM_DRUM_VOICES] = {
  mini_acid_.patternKickSteps(),
 ...
 for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
  ...
  if (instrumentDelta > 0) {
    srcVoice = (v + 1) % NUM_DRUM_VOICES; // up: content from next voice wraps into current
  } else {
    srcVoice = (v - 1 + NUM_DRUM_VOICES) % NUM_DRUM_VOICES; // down: content from previous voice
  }
  for (int i = 0; i < SEQ_STEPS; ++i) {
    target_hits[v][i] = current_hits[srcVoice][i];
    target_accents_v[v][i] = current_accents_v[srcVoice][i];
  }
 }
 withAudioGuard([&]() {
  for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
...
void DrumSequencerMainPage::copyFirstHalfToSecondHalf() {
 bool current_hits[NUM_DRUM_VOICES][SEQ_STEPS];
 bool current_accents_v[NUM_DRUM_VOICES][SEQ_STEPS];
 bool current_accent_global[SEQ_STEPS];
 const bool* hits_src[NUM_DRUM_VOICES] = {
 ...
 withAudioGuard([&]() {
  for (int v = 0; v < NUM_DRUM_VOICES; ++v) {
    for (int i = 8; i < SEQ_STEPS; ++i) {
      int src = i - 8;
      bool desiredHit = current_hits[v][src];
      bool desiredAccentV = current_accents_v[v][src] && desiredHit;
  ...
  for (int i = 8; i < SEQ_STEPS; ++i) {
    int src = i - 8;
    if (current_accent_global[i] != current_accent_global[src]) {
      mini_acid_.toggleDrumAccentStep(i);
    }
  }
 });
 ...
 */
 ...
 case 'g': { // transpose instruments up
  transposePattern(+1);
  return true;
 }
 case 'b': { // transpose instruments down
  transposePattern(-1);
  return true;
 }
 case '\'': { // copy steps 0-7 into 8-15
...
GlobalDrumSettingsPage::GlobalDrumSettingsPage(MiniAcid& mini_acid)
 ...
 if (drum_engine_options_.empty()) {
  drum_engine_options_ = {"808", "909", "606", "202"};
 }
